...
RUN pip install --no-cache-dir -r /tmp/requirements.txt gunicorn yt-dlp

COPY . /app

EXPOSE 8085
CMD ["sh", "-c", "gunicorn media_service.wsgi:application --bind 0.0.0.0:8085 --workers \"${MEDIA_SERVICE_WORKERS:-2}\" --threads \"${MEDIA_SERVICE_THREADS:-4}\" --timeout \"${MEDIA_SERVICE_REQUEST_TIMEOUT:-1800}\" --max-requests \"${MEDIA_SERVICE_MAX_REQUESTS:-500}\" --max-requests-jitter \"${MEDIA_SERVICE_MAX_REQUESTS_JITTER:-50}\" --access-logfile - --error-logfile -"]
